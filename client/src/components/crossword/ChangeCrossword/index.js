...
const ChangeCrossword = () => {
  ...
  useEffect(() => {
    if (crossword && width && height) {
      ...
      crossword.forEach(function (notion) {
        if (notion.direction === -1) {
          for (let k = notion.i; k < notion.word.length + notion.i; k++) {
            table
              .item(k)
              .childNodes.item(notion.j)
              .classList.remove('table__cell_empty');
            table
              .item(k)
              .childNodes.item(notion.j)
              .classList.add('table__cell_not-empty');
            table.item(k).childNodes.item(notion.j).textContent =
              notion.word[k - notion.i];
            table.item(k).childNodes.item(notion.j).ans =
              notion.word[k - notion.i];
          ...
          for (let k1 = notion.j; k1 < notion.word.length + notion.j; k1++) {
            table
              ...
              .childNodes.item(k1)
              .classList.add('table__cell_not-empty');
            table.item(notion.i).childNodes.item(k1).textContent =
              notion.word[k1 - notion.j];
            table.item(notion.i).childNodes.item(k1).ans =
              notion.word[k1 - notion.j];
  ...
  }, [crossword, width, height]);

  const onCellClick = (e) => {
    console.log(e);
    if (e.target.words !== undefined) {
      if (e.target.key === undefined) e.target.key = 0;
      e.target.key += 1;
      if (
  ...
  const onCheckClick = (e) => {
    ...
    document.querySelectorAll('.table__cell_not-empty').forEach((el) => {
      ...
      if (el.textContent.toLowerCase() == el.ans.toLowerCase()) {
        ...
        if (!(el.defs[0] in correct)) correct[el.defs[0]] = 0;
        correct[el.defs[0]] += 1;
        if (correct[el.defs[0]] == 2) corAns += 1;
      } else {
